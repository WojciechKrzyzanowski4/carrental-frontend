  ...
  function handleSubmit(event) {
    ...
    fetch(SERVER_URL + "/offer/edit", requestOptions).then(async (response) => {
      if (!response.ok) {
        ...
        alert(
          "the other admin was faster and managed to delete the offer before you could edit it"
        );
      } else {
        alert("offer edited");
      }
      ...
      >
        <div className="modal-container mx-auto p-16 bg-white rounded-lg shadow-lg">
          ...
          <form onSubmit={handleSubmit}>
            ...
            />
            <select
              className="block mb-4 p-2 border border-gray-300 rounded width-[100%]"
              name="carId"
              defaultValue={offer.carId}
            >
              ...
              {cars.map((car) => (
                <option
                  key={car.id}
                  value={car.id}
                >{`${car.brand} ${car.model} (${car.year})`}</option>
              ))}
            </select>
